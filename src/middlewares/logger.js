...
const loggerMiddleware = async (ctx, next) => {
  ...
  let logText = `${ctx.method} ${ctx.status} ${ctx.url} ${JSON.stringify(ctx)} ${JSON.stringify(ctx.request)} 请求参数： ${JSON.stringify(ctx.request.body)} 响应参数： ${JSON.stringify(ctx.body)} - ${remoteAddress} - ${ms}ms`;
